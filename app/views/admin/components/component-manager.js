...
class ComponentManager {
    ...
    configureSidebar(config) {
        ...
        if (config.activePage) {
            sidebar.querySelectorAll('.sidebar-nav-link').forEach(link => {
                ...
                if ((config.activePage === 'customers' || config.activePage === 'customer-roles') && 
    ...
    configureTableCard(container, config) {
        ...
        if (config.headers) {
            ...
            if (headerContainer) {
                headerContainer.innerHTML = `<tr>${config.headers.map(header => `<th>${header}</th>`).join('')}</tr>`;
            }
        }
    ...
    async initializePage(pageConfig) {
        ...
        if (pageConfig.components) {
            for (const [componentType, configs] of Object.entries(pageConfig.components)) {
                if (Array.isArray(configs)) {
                    configs.forEach((config, index) => {
                        const container = document.querySelector(config.container || `#${componentType}-${index}`);
                        if (container) {
                            this[`render${componentType.charAt(0).toUpperCase() + componentType.slice(1)}`](container, config);
                        }
                    });
                }
            }
